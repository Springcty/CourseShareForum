...
<template>
  <el-card class="main-card">
    ...
    <el-table border :data="commentList" @selection-change="selectionChange" v-loading="loading">
      ...
      <!-- 评论人昵称 -->
      <el-table-column prop="username" label="评论人" align="center" width="120" />
      <!-- 回复人昵称
      <el-table-column
        prop="replyNickname"
        label="回复人"
...
export default {
  ...
  methods: {
    ...
    deleteComments(id) {
      if (id != null) {
        this.axios
          .delete(`http://101.37.254.247:9001/admin/comments/deleteId/${id}`)
          .then(({ data }) => {
            if (data.flag) {
              this.$notify.success({
                title: "成功",
        ...
        this.axios
          .post(
            "http://101.37.254.247:9001/admin/comments/deleteIds",
            this.commentIdList
          )
          .then(({ data }) => {
            if (data.flag) {
    ...
    listComments() {
      ...
      this.axios
        .post("http://101.37.254.247:9001/admin/commentsList", param)
        .then(({ data }) => {
